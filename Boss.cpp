...
int Boss::attackPlayer() {
	...
	if (UseBossSkill <= 55) // 확률 분기
	{
		cout << "\n  " << "\033[1;31m" << Name << "이 당신을 공격합니다!" << "\033[0m" << std::endl; // 55% 일반 공격
		player->takeDamage(getAttack());
	}
	else { // 45% 확률로 가드 이벤트 발생
		int currentPotions = Potion::countTotalPotions(player->getInventory());

		GuardSummary summary = BossAttackGuard(currentPotions);

		// 모든 타격을 막았을 때만 보스에게 반격
		if (summary.allSuccess && player->getHealth() > 0) {
			int counterDamage = player->getAttack() * summary.successCount; // 가드를 성공한 횟수만큼 반격 데미지 곱하기
			cout << "\n  \033[1;36m[카운터] 완벽한 가드! 보스에게 강력한 반격을 날립니다!\033[0m" << endl;
			cout << "  보스에게 " << counterDamage << "의 데미지!" << endl;
			this->takeDamage(counterDamage);
			player->setHealth(player->getHealth() + counterDamage); // 반격 데미지만큼 체력 회복
			cout << "\n  \033  의지가 함양되어 체력이 " << counterDamage << " 만큼 회복되었습니다!\033[0m" << endl;
...
GuardSummary Boss::BossAttackGuard(int& potionCount) {
	...
	bool isEnraged = (getHealth() < (getMaxHealth() / 2)); // 체력 50% 미만 일때 광폭화

	// 일반: 1.0초 / 광폭화: 0.4초 (판정 시간)
	int judgeTime = isEnraged ? 400 : 1000;

	// 일반: 500~2500ms / 광폭화: 300~1200ms (대기 시간)
	int minWait = isEnraged ? 300 : 500;
	int maxWait = isEnraged ? 1200 : 2500;

	uniform_int_distribution<> disCount(2, 4); //최소, 최대 연속 공격 횟수
...
void Boss::showPatternGuide() {
	...
	cout << "                   ==================================================" << endl;
	cout << "              보스가 기를 모은 뒤 랜덤한 시간 뒤 화면이 \033[1;31m빨간색\033[0m으로 변합니다!" << endl;
